...
- (void)setMode:(MBProgressHUDMode2)newMode {
 ...
 if (mode == MBProgressHUDModeDeterminate2) {
 if ([NSThread isMainThread]) {
 [self updateProgress];
 [self setNeedsDisplay];
 } else {
 ...
 - (void)updateProgress {
 [(MBRoundProgressView2 *)indicator setProgress:progress];
 }
 ...
 if (mode == MBProgressHUDModeDeterminate2) {
 self.indicator = [[[MBRoundProgressView2 alloc] init] autorelease];
 }
 else if (mode == MBProgressHUDModeCustomView2 && self.customView != nil){
...
+ (MBProgressHUD2 *)showHUDAddedTo:(UIView *)view
                              text:(NSString*)labelText
                      detailedText:(NSString*)detailedLabelText
                          animated:(BOOL)animated {
    MBProgressHUD2 *hud = [[MBProgressHUD2 alloc] initWithView:view];
    hud.labelText = labelText;
    hud.detailsLabelText = detailedLabelText;
    [view addSubview:hud];
    [hud show:animated];
 ...
 if ([v isKindOfClass:[MBProgressHUD2 class]]) {
 viewToRemove = v;
 }
 }
 if (viewToRemove != nil) {
 MBProgressHUD2 *HUD = (MBProgressHUD2 *)viewToRemove;
 ...
 self.animationType = MBProgressHUDAnimationFade2;
 self.mode = MBProgressHUDModeIndeterminate2;
 self.labelText = nil;
...
- (void)hide:(BOOL)animated afterDelay:(NSTimeInterval)delay {
 ...
 - (void)showUsingAnimation:(BOOL)animated {
 self.alpha = 0.0f;
 if (animated && animationType == MBProgressHUDAnimationZoom2) {
 ...
 self.alpha = 1.0f;
 if (animationType == MBProgressHUDAnimationZoom2) {
...
@implementation MBRoundProgressView2
