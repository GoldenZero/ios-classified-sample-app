...
- (void)textFieldDidBeginEditing:(UITextField *)aTextField {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        if (aTextField.tag == 50) {
            [self.inputAccessoryView setHasDoneButton:YES];
        }else
        {
            [self.inputAccessoryView setHasDoneButton:NO];
        }
    ...
    if (self.iPad_countryPopOver) {
        ...
        for (int i =0; i <= [countryArray count] - 1; i++) {
            if ([(Country *)[countryArray objectAtIndex:i] countryID] == defaultCountryID)
            {
                ...
                [_countryCity setTitle:temp forState:UIControlStateNormal];
                break;
                //return;
            }
