...
- (void)viewDidLoad
{
    ...
    [self.iPad_chooseBrandBtn setEnabled:NO];
...
- (void) customizeButtonsInXib {
    ...
    [self.theStore setTitle:@"اختر المتجر" forState:UIControlStateNormal];
    // TODO set the Store to the current
    if (self.currentStore) {
        [self.theStore setTitle:self.currentStore.name forState:UIControlStateNormal];
    ...
    [carDetails setText:myAdInfo.description]; //TODO get Description
    ...
    while (defaultUnitIndex<unitsArray.count) {
        if (defaultUnitID==[(SingleValue*)[unitsArray objectAtIndex:defaultUnitIndex] valueID]) {
            UnitString =[NSString stringWithFormat:@"%@",[(SingleValue*)[unitsArray objectAtIndex:defaultUnitIndex] valueString]];
            break;
        }
        defaultUnitIndex++;
    }
    [_units setTitle:UnitString forState:UIControlStateNormal];
    
    [_propertySpace setText:myAdInfo.SpaceString];
    
    [mobileNum setText:myAdInfo.mobileNumber]; //TODO get mobile number
...
- (void) setImagesArray{
    ...
    for (int i=0; i<6; i++) {
        if ([self.myImageIDArray count] == 0 || remainingImg == 0) {
            ...
            UIButton * delBtn = (UIButton *) [self.iPad_uploadPhotosView viewWithTag:((i+1) *100)];
            [delBtn setHidden:YES];
            cardADS = (Ad*)[self.myImageIDArray objectAtIndex:i];
 ...
 /*   //check year
   ...
   */
    ...
    if (self.browsingForSale) {
        // post for sale
        [[AdsManager sharedInstance] editAdStoreForSaleOfEditadID:self.myDetails.EncEditID myStore:myStore.identifier OfCategory:self.myDetails.CategoryID inCountryID:chosenCountry.countryID InCity:chosenCity.cityID userEmail:savedProfile.emailAddress title:carAdTitle.text description:carDetailLabel.text adPeriod:chosenPeriod.valueID requireService:RequiredService price:carPrice.text currencyValueID:chosenCurrency.valueID unitPrice:self.unitPrice.text unitType:chosenUnit.valueID imageIDs:currentImgsUploaded longitude:[NSString stringWithFormat:@"%f",PropertyLocation.coordinate.longitude] latitude:[NSString stringWithFormat:@"%f",PropertyLocation.coordinate.latitude] roomNumber:(chosenRoom.valueString ? chosenRoom.valueString : @"") space:self.propertySpace.text area:self.propertyArea.text mobile:self.mobileNum.text phoneNumer:self.phoneNum.text withDelegate:self];
    ...
    }else
    {
        ...
        [[AdsManager sharedInstance] editAdStoreForRentOfEditadID:self.myDetails.EncEditID myStore:myStore.identifier OfCategory:self.myDetails.CategoryID inCountryID:chosenCountry.countryID InCity:chosenCity.cityID userEmail:savedProfile.emailAddress title:carAdTitle.text description:carDetailLabel.text adPeriod:chosenPeriod.valueID requireService:RequiredService price:self.carPrice.text currencyValueID:chosenCurrency.valueID unitPrice:self.unitPrice.text unitType:chosenUnit.valueID imageIDs:currentImgsUploaded longitude:[NSString stringWithFormat:@"%f",PropertyLocation.coordinate.longitude] latitude:[NSString stringWithFormat:@"%f",PropertyLocation.coordinate.latitude] roomNumber:(chosenRoom.valueString ? chosenRoom.valueString : @"") space:self.propertySpace.text area:self.propertyArea.text mobile:self.mobileNum.text phoneNumer:self.phoneNum.text withDelegate:self];
...
- (void) iPad_setStepViews {
    ...
    for (int i = 0; i < [currentCategory count]; i++) {
        ...
        UILabel* CatLbl = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 215, 50)];
