//
//  CarAdDetailsViewController_iPad.m
//  Bezaat
//
...
@implementation CarAdDetailsViewController_iPad
...
- (IBAction)sendMailBtnPrss:(id)sender {
    ...
    if (![currentDetailsObject.emailAddress isEqualToString:@""])
    {
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
            ...
            [self presentViewController:vc animated:YES completion:nil];
            //vc.view.superview.frame = CGRectMake(200, 0, 400, 500);
            //vc.view.superview.bounds = CGRectMake(0, 0, 400, 500);
...
- (void) iPad_resizeScrollView {
    ...
    if (currentDetailsObject)
    {
        ...
        if (![currentDetailsObject.description isEqualToString:@""]) {
            ...
            label = [[UILabel alloc] initWithFrame:CGRectMake(
                                                              self.detailsTextLabel.frame.origin.x - 9,
                                                              self.detailsTextLabel.frame.origin.y + self.detailsTextLabel.frame.size.height + 5,
                                                              expectedLabelSizeNew.width,
                                                              expectedLabelSizeNew.height + 10)];
            ...
            [self.iPad_detailsView setFrame:CGRectMake(iPad_detailsVieworiginalFrame.origin.x,
                                                       ...
                                                       self.detailsTextLabel.frame.origin.y + self.detailsTextLabel.frame.size.height + label.frame.size.height + 5 + 10)];
        ...
        if ((currentDetailsObject.attributes) && (currentDetailsObject.attributes.count))
        {
            ...
            for (AdDetailsAttribute * attr in currentDetailsObject.attributes)
            {
                if (attr.categoryAttributeID == 10091){
                    if ([attr.attributeValue length] != 0) {
                        ...
                        [self.iPad_videoBtn setHidden:NO];
            ...
            for (AdDetailsAttribute * attr in currentDetailsObject.attributes)
            {
                if (attr.categoryAttributeID == 1005 || attr.categoryAttributeID == 405) {
                    self.periodLabel.text = attr.attributeValue;
                }
                if (attr.categoryAttributeID != 10094 &&
                    ...
                    attr.categoryAttributeID != 1005 &&
                    attr.categoryAttributeID != 405) {
                    
                    if ([attr.attributeValue length] != 0) {
                        
                        if (attr.categoryAttributeID == 1019 || attr.categoryAttributeID == 851 || attr.categoryAttributeID == 1022) {
                            self.yearMiniLabel.text = [NSString stringWithFormat:@"(%@) غرف",attr.attributeValue];
                            [self.yearMiniLabel setHidden:NO];
                            [self.yearMiniImg setHidden:NO];
                        }
                        if (attr.categoryAttributeID ==  1050|| attr.categoryAttributeID == 1049) {
                            self.kiloMiniLabel.text = attr.attributeValue;
                            [self.kiloMiniLabel setHidden:NO];
                            [self.kiloMiniImg setHidden:NO];
                        }
                        //attr label
                        NSString * attr_name_text = [NSString stringWithFormat:@"%@ :", attr.displayName];
                        ...
                        if ([attr.displayName isEqualToString:@"رقم الجوال"] || [attr.displayName isEqualToString:@"رقم الهاتف"]) {
                            phoneBtn = [UIButton buttonWithType:UIButtonTypeCustom];
                            //phoneBtn.frame = CGRectMake(30, 1, 128, 24);
                            phoneBtn.frame = CGRectMake(10, 6, valueLabelWidth, 21);
                            NSLog(@"width of phone : %f",valueLabelWidth);
                            // CGSize size = [attr.attributeValue sizeWithFont:[UIFont systemFontOfSize:15.0f] constrainedToSize:CGSizeMake(valueLabelWidth + 20, valueLabelHeight) lineBreakMode:UILineBreakModeCharacterWrap];
                            //CGRect frame = CGRectMake(20, 1, size.width, 24);
                            //phoneBtn.frame = frame;
                        ...
                        [self.labelsScrollView setContentSize:
                         (CGSizeMake(self.labelsScrollView.frame.size.width, ((addedHeightValue > self.iPad_detailsView.frame.size.height + 20) ? addedHeightValue : self.iPad_detailsView.frame.size.height + 40)))];
            ...
            if ([self.periodLabel.text length] == 0) {
                self.periodMiniImg.hidden = YES;
                self.periodLabel.hidden = YES;
            }
            
            if (currentDetailsObject.latitude == 0.0 || currentDetailsObject.longitude == 0.0) {
            ...
            region.span = span;
            [self.iPad_locationMap setRegion:region animated:YES];
            ...
            [self.iPad_locationMap addAnnotation:dropPin];
            ...
            [self.labelsScrollView setContentSize:
             (CGSizeMake(self.labelsScrollView.frame.size.width, ((totalHeight > self.iPad_detailsView.frame.size.height + 20) ? totalHeight : self.iPad_detailsView.frame.size.height + 40)))];
...
- (void) customizeButtonsByData {
    ...
    if (currentDetailsObject)
    {
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
            if (currentDetailsObject.isFeatured)
            {
                if (currentStore && (currentDetailsObject.storeID == currentStore.identifier))
                {
                    ...
                    [self.view setBackgroundColor:[UIColor colorWithRed:249.0/255 green:231.0/255 blue:219.0/255 alpha:1.0f]];
                }
                else
        ...
        if(savedProfile){
            ...
            // Check favorite
            if (currentDetailsObject.isFavorite) {
                [self.favoriteButton setImage:[UIImage imageNamed:(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? @"Details_navication_2_hart.png" : @"iPad_details_fav_on.png")] forState:UIControlStateNormal];
            }
        ...
        if (currentDetailsObject.isFeatured) {
            if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
                ...
                [self.pageControl setTintColor:[UIColor grayColor]];
                ...
                self.priceLabel.frame = CGRectMake(self.priceLabel.frame.origin.x + 20, self.priceLabel.frame.origin.y, self.priceLabel.frame.size.width, self.priceLabel.frame.size.height);
                [self.view setBackgroundColor:[UIColor colorWithRed:249.0/255 green:231.0/255 blue:219.0/255 alpha:1.0f]];
            }
...
- (void) detailsDidFinishLoadingWithData:(AdDetails *) resultObject {
    ...
    if (currentDetailsObject)
    {
        ...
        if (currentDetailsObject.storeID == 0) {
            self.iPad_firstTitleView.frame = CGRectMake(self.iPad_firstTitleView.frame.origin.x, self.iPad_firstTitleView.frame.origin.y - 45, self.iPad_firstTitleView.frame.size.width, self.iPad_firstTitleView.frame.size.height + 45);
        }
        ...
        if ([currentDetailsObject.Rooms length] > 0)
            ...
            self.yearMiniLabel.hidden = NO;
            self.yearMiniImg.hidden = NO;
        }
        ...
        if (currentDetailsObject.SpaceString)
            ...
            self.kiloMiniLabel.hidden = NO;
            self.kiloMiniImg.hidden = NO;
        }
        ...
        if (currentDetailsObject.viewCount > 0)
            ...
            xForShiftingTinyImg = self.countOfViewsTinyImg.frame.origin.x;
...
- (void) FavoriteFailAddingWithError:(NSError*) error forAdID:(NSUInteger)adID {
    ...
    [GenericMethods throwAlertWithTitle:@"خطأ" message:[error description] delegateVC:self];
    [self.favoriteButton setImage:[UIImage imageNamed:(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? @"Details_gray_heart.png" : @"iPad_details_fav_off.png")] forState:UIControlStateNormal];
}
...
- (IBAction)iPad_showCommentPressed:(id)sender {
    //load comments page
    CommentsPopUpViewController *vc =[[CommentsPopUpViewController alloc] initWithNibName:@"CommentsPopUpViewController" bundle:nil];
    vc.AdID = currentAdID;
    vc.modalPresentationStyle = UIModalPresentationFormSheet;
    [self presentViewController:vc animated:YES completion:nil];
    vc.view.superview.frame = CGRectMake(0, 0, 589, 500);
    //vc.view.superview.bounds = CGRectMake(0, 0, 600, 600);
    vc.view.superview.center = CGPointMake(roundf(self.view.bounds.size.width / 2), roundf(self.view.bounds.size.height / 2));
...
- (void) commentsDidFailLoadingWithError:(NSError *)error {
    [self hideLoadingIndicator];
    [self.iPad_showComments setHidden:YES];
