...
#define UNIT_TYPE_FILE_NAME         @"units.json"
...
//-----------------------------------
#pragma mark - UNIT_TYPE json keys

#define UNIT_TYPE_CAT_ATTR_MASTER_VALUE_ID_JKEY     @"CategoryAttributeMasterValueID"
#define UNIT_TYPE_MASTER_VALUE_JKEY                 @"MasterValue" //the display name
#define UNIT_TYPE_DISPLAY_ORDER_JKEY                @"DisplayOrder"
...
- (NSArray *) loadRoomsValues
{
    ...
    if (!roomsValues)
    {
        NSData * data = [NSData dataWithContentsOfFile:[self getJsonFilePathInDocumentsForFile:ADD_ROOMS_FILE_NAME]];
...
- (NSArray *) loadUnitValues {
    ...
    if (!UnitValues)
    {
        NSData * data = [NSData dataWithContentsOfFile:[self getJsonFilePathInDocumentsForFile:UNIT_TYPE_FILE_NAME]];
        ...
        for (NSDictionary * dict in parsedArray)
        {
            //create value object
            SingleValue * val = [[SingleValue alloc]
                                 initWithValueIDString:[dict objectForKey:UNIT_TYPE_CAT_ATTR_MASTER_VALUE_ID_JKEY]
                                 valueString:[dict objectForKey:UNIT_TYPE_MASTER_VALUE_JKEY]
                                 displayOrderString:[dict objectForKey:UNIT_TYPE_DISPLAY_ORDER_JKEY]
                                 ];
            [result addObject:val];
        }
        NSArray * temp = [self sortValuesArray:result];
